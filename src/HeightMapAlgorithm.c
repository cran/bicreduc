...
SEXP HeightMapAlgorithm(SEXP RR, SEXP BB)
{
    ...
    /* 4. and 5. set h_1..h_{2n} and e_1..e_{2n} to zero */
    for (i=1; i<=2*n; i++)
    {
        h[i] = 0; e[i] = 0;
    }
    ...
    for (j=1; j<=2*n; j++)
    {
        ...
        else 
        {
            ...
            for (k=Y1(r[j]) + 1; k<=Y2(r[j]) - 1; k++)
            {
                ...
                if ((h[k+1]<h[k]) && (b>0))
                {
                    /* 14. if (e_{b+1},...,e_k > 0) then */
					for (i=b+1; (i<=k && (e[i]>0)); i++);		/* empty loop */
                    if (i>k)
                    {
            ...
            /* 25. for k = y_{1,r_j} + 1 to y_{2,r_j} do */
            for (k=Y1(r[j]) + 1; k<=Y2(r[j]); k++)
            {
                /* 26. h_k := h_k - 1 */
                h[k]--;
